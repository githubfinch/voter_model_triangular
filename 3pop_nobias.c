...
int main()
{
	// srand(time(0));
	srand(10);
	int number_row = 50; // must be EVEN
	int number_column = 25; 
...
// WRITE TO FILES
	FILE *file = fopen("states_BASIC.txt","w");
	if (file == NULL)
	{
		perror("ERR OPEN FILE\n");
		return 1;
	}
	FILE *three_pop_FILE = fopen("3_pop_BASIC.txt","w");
...
// LINK UP SAME VALUED ENTRIES, for specific valued entry see later
	...
	while (list_length(linked_rivals) != 0)
	{
		...
		// ANOTHER BUNCH OF INIT
		int select_index = get_list_data(linked_rivals,rand_select);
		int* temp_neighbourhood = get_neighbours(select_index, number_row, number_column);
		int* temp_rival_list = malloc(6 * sizeof(int));
		int temp_rival_count = 0;

		// CHOSEN ONE IS CONFORMED
		if (rand_select % 2 == 0)
		{
			...
			all_states[select_index] = all_states[select_index_NEXT];
		...
		else if (rand_select % 2 == 1)
		{
			...
			all_states[select_index] = all_states[select_index_PREV];
		...
		int count;
		int* chain_of_indices = isolate_list(linked_rivals, select_index, &count);
		...
		for(int n = 0; n < 3; n++) // iterate through single neighbourhood
		{
			if (all_states[temp_neighbourhood[n]] != all_states[select_index])
			{
				temp_rival_list[temp_rival_count] = select_index;
